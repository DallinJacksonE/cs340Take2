...
export class DynamoStatusDAO extends DynamoDAO implements StatusDAO {
	...
	public async putFeedBatch(
		...
		for (let i = 0; i < userAliases.length; i += batchSize) {
			...
			while (keepTrying) {
				// 1. Send the batch
				const response = await this._documentClient.send(
					new BatchWriteCommand(params),
				);

				// 2. Check if AWS throttled and rejected any items
				if (
					response.UnprocessedItems &&
					Object.keys(response.UnprocessedItems).length > 0
				) {
					// Pause for 50ms to let DynamoDB capacity refill before retrying
					await new Promise((resolve) => setTimeout(resolve, 50));

					// Reassign params to ONLY retry the items that failed
					params.RequestItems = response.UnprocessedItems;
				} else {
					// The array is empty, all items in this 25-pack succeeded!
					keepTrying = false;
				}
			}
		}
